...
switch (_blck_WorldName) do {// These may need some adjustment - including a test for shore or water should help as well to avoid missions spawning on water.
	case "altis":{
		...
		if (blck_blacklistSpawns) then {
			[1,"Spawn black list locations added for Altis"] call beef_fncUtil_Log;
	...
	case "stratis":{
		...
		if (blck_blacklistSpawns) then {
			...
			[1,"Spawn black list locations added for Stratis"] call beef_fncUtil_Log;
		};
	}; // Add Central, East and West respawns/traders
	case "chernarus":{
		...
		if (blck_blacklistSpawns) then {
			...
			[1,"Spawn black list locations added for Chernarus"] call beef_fncUtil_Log;
		};
	};
	...
	case "bornholm":{
		...
		if (blck_blacklistSpawns) then {
			...
			[1,"Spawn black list locations added for Bornholm"] call beef_fncUtil_Log;
		};
	};
	case "esseker":{
		...
		if (blck_blacklistSpawns) then {
			...
			[1,"Spawn black list locations added for Esseker"] call beef_fncUtil_Log;
		};
	};
	case "australia":{
		blck_mapCenter = [20480,20480, 150];blck_mapRange = 40960;
			...
			[1,"Spawn black list locations added for Australia"] call beef_fncUtil_Log;
			};
		};  //
	...
	case "takistan":{
		...
		if (blck_blacklistSpawns) then {
			...
			[1,"Spawn black list locations added for Takistan"] call beef_fncUtil_Log;
			};
		};
	default {
...
if (_blck_WorldName == "default") then {
	...
	[0,format["Worldname is %1. Map specific settings defined",_blck_WorldName]] call beef_fncUtil_Log;
};
